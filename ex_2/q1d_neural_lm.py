...
def lm_wrapper(in_word_index, out_word_index, num_to_word_embedding, dimensions, params):
    ...
    ### YOUR CODE HERE
    indexes = random.sample(range(in_word_index.__len__()), BATCH_SIZE)
    in_embedding = [num_to_word_embedding[idx] for i, idx in enumerate(in_word_index) if i in indexes]
    out = [out_word_index[idx] for idx in indexes]
    for i in range(BATCH_SIZE):
        labels[i, out[i]] = 1
    #out_embedding = [num_to_word_embedding[idx] for i, idx in enumerate(out_word_index) if i in indexes]

    data = np.array(in_embedding)
...
def eval_neural_lm(eval_data_path):
    ...
    ### YOUR CODE HERE
    bs = 50
    labels = np.zeros([bs, output_dim])
    for i in range(0, bs, num_of_examples):
        indexes = range(i, i + bs)
        in_embedding = [num_to_word_embedding[idx] for i, idx in enumerate(in_word_index) if i in indexes]
        data = np.array(in_embedding)
        out = [out_word_index[idx] for idx in indexes]
        labels[:, out[i: i + bs]] = 1

        out = forward(data, labels, params, dimensions)
        p = np.power(2, -np.mean(np.log2(out), axis=-1))
        perplexity += np.sum(p)
...
if __name__ == "__main__":
    ...
    params = sgd(
            ...
            params, LEARNING_RATE, NUM_OF_SGD_ITERATIONS, None, True, 10)  # TODO
