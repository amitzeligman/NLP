...
def forward_backward_prop(data, labels, params, dimensions):
    ...
    ### YOUR CODE HERE: forward propagation
    y = forward(data, labels, params, dimensions)

    cost = -np.sum(np.multiply(labels, np.log(y)))
    ### END YOUR CODE
    ...
    gradW1 = np.dot(data.T, (np.multiply(np.dot(diff, W2.T), sigmoid_grad(h))))
    gradb1 = np.multiply(np.dot(diff, W2.T), sigmoid_grad(h)).sum(axis=0, keepdims=True)
    gradW2 = np.dot(h.T, diff)
    gradb2 = diff.sum(axis=0, keepdims=True)
