...
def load_data(args, helper=None):
    ...
    if hasattr(args, 'data_train') and helper is None:
        ...
        helper.max_length = max(max(len(sentence) for sentence, _ in dev_examples['tokens']), helper.max_length)

    # now process all the input data.
    if hasattr(args, 'data_train'):
        train_examples['token_indices'] = helper.vectorize(train_examples['tokens'])
    if hasattr(args, 'data_dev'):
        dev_examples['token_indices'] = helper.vectorize(dev_examples['tokens'])
    if hasattr(args, 'data'):
        data_examples['token_indices'] = helper.vectorize(data_examples['tokens'])
...
def load_embeddings(args, helper, device='cpu'):
